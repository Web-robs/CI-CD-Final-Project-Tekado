...
const seedDB = async ({ force = false, skipIfExists = true } = {}) => {
  ...
  const result = await prisma.product.createMany({ data: productSeeds });
  console.log(`Products data seeded successfully! Inserted ${result.count} records.`);

  return { seeded: true, skipped: false, inserted: result.count };
};

// Only runs when the "node productSeeds.js dev" command is executed manually
if (process.argv[2] == 'dev') {
  dotenv.config({ path: path.resolve(__dirname, '../.env') });
  seedDB({ force: true, skipIfExists: false })
    .catch(err => {
