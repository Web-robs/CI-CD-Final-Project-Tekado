...
module.exports = async function syncPinecone() {
  if (!DATABASE_URL) {
    throw new Error('DATABASE_URL must be set to sync Pinecone');
  }
  ...
  for (const batch of batches) {
    ...
    for (const doc of batch) {
      ...
      try {
        ...
        const idString = (doc.pineconeId || doc.id).toString();
        vectors.push({
          id: idString,
          values: embedding,
          metadata: {
        ...
        console.warn(`⚠️  Embedding failed for ${doc.id}:`, err.message || err);
      }
    }
    ...
    if (syncedIds.length) {
      const updates = syncedIds.map(id =>
        prisma.product.update({
          where: { id: Number(id) },
          data: { pineconeId: id.toString() },
        })
      );
